...
const showDevTools =
    Boolean(process.env.SHOW_DEV_TOOLS) ||
    process.argv.indexOf('--show-dev-tools') > -1;
...
function createJitsiMeetWindow() {
    ...
    const indexURL = URL.format({
        pathname: path.resolve(basePath, './renderer/index.html'),
        protocol: 'file:',
        slashes: true,
    });
    ...
    const options = {
        ...
        webPreferences: {
            ...
            preload: path.resolve(basePath, './renderer/preload.js'),
            sandbox: false,
        },
    };
    ...
    mainWindow.webContents.session.webRequest.onHeadersReceived(
        (details, callback) => {
            ...
            if (details.responseHeaders['content-security-policy']) {
                const cspFiltered = details.responseHeaders[
    ...
    setupScreenSharingMain(mainWindow, config.appName, pkgJson.build.appId);
...
const gotInstanceLock =
    process.platform === 'darwin' ? true : app.requestSingleInstanceLock();
...
if (isDev && process.platform === 'win32') {
    ...
    app.setAsDefaultProtocolClient(config.appProtocolPrefix, process.execPath, [
